...
//---------------------------------------------------------------------------------function for Smartcontract class
//---------------------------------------------------------------------------------method that you can query from audit log
...
//---------------------------------------------------------------------------------helper method to implement line 108
//---------------------------------------------------------------------------------there are 13 argument in line 50
func (s *SmartContract) RecordUpdate(APIstub shim.ChaincodeStubInterface, args []string) sc.Response {
	if len(args) != 7 {
		return shim.Error(fmt.Sprintf("Expecting 7 arguments; got %d", len(args)))
	}
	...
	//---------------------------------------------------------------------------------how to handle the error(json.marshal) create event begin at 209
	...
	return shim.Success([]byte("Your access has been recorded"))
}

//---------------------------------------------------------------------------------helper method that convert event to array
...
//---------------------------------------------------------------------------------helper method that split 13 argument"ActionType : XXX""userID: XXX"
...
func (s *SmartContract) CreateRecordEvent(args []string) (RecordEvent, error) {
	...
	recordEvent := RecordEvent{
		...
		TransactionID: stringMap["transactionId"],
		GeneID:        stringMap["geneId"],
		Time:          time,
	}
...
func (s *SmartContract) IsValidField(field string) bool {
	switch field {
	case
		"actionType", "userId", "patientId", "recordId", "transactionId", "geneId", "time":
		return true
	}
	return false
}

//---------------------------------------------------------------------------------check if id exist,if not append it
...
//---------------------------------------------------------------------------------helper method to traversal time
//---------------------------------------------------------------------------------line331 order of element in four structs different
